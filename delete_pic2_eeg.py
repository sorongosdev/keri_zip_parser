                    ...
                    if os.path.isdir(case_folder_path):
                        for file_name in os.listdir(case_folder_path):
                            if file_name.endswith('.zip'):
                                ...
                                delete_txt_from_zip(zip_path)

def delete_txt_from_zip(zip_path):
    ...
    # Delete txt files ending with '_2.txt'
    for root, _, files in os.walk(temp_dir):
        for file in files:
            if file.endswith('_2_EEG.txt'):
                file_path = os.path.join(root, file)
                os.remove(file_path)
                print(f"Deleted file: {file_path}")
    
    # Recreate the zip file without the deleted txt files
...
# 사용 예시
root_directory = 'delete_pic2_eeg'
